        ...
        const participantsListItems = details.participants
          .map(
            (participant) =>
              ...
              `<button type="button" class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}" aria-label="Unregister ${escapeHtml(participant)}">&times;</button>` +
              `</li>`
          )
          .join("");
